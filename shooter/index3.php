...
<body class="d-flex flex-column h-100 bg-dark">
    ...
    <?php
    // $date_now = date_create("2022-10-04");
    // $date_now = date_create(date("Y-m-d"));
    $day_before_match = 13;
    $day_after_match = 15;
    $date_before_match = date_create(date("Y-m-d"));
    date_add($date_before_match, date_interval_create_from_date_string($day_before_match . " days"));
    $date_after_match =  date_create(date("Y-m-d"));
    date_add($date_after_match, date_interval_create_from_date_string("-" . $day_after_match . " days"));
    // echo "today: " . date_format($date_now,"Y-m-d") . "<br>";
    // echo "date_before: " . date_format($date_before_match,"Y-m-d") . "<br>";
    // echo "date_after: " . date_format($date_after_match,"Y-m-d") . "<br>";
    // $sql = "SELECT * FROM `match_idpa` WHERE `match_begin` >= '" . date_format($date_after_match,"Y-m-d") . "' AND `match_begin` <= '" . date_format($date_before_match,"Y-m-d") . "'";
    $sql = "SELECT * FROM match_idpa INNER JOIN match_setting mase ON mase.match_id = match_idpa.match_id WHERE mase.match_active = 'true' AND match_idpa.match_begin >= '" . date_format($date_after_match, "Y-m-d") . "' AND match_idpa.match_begin <= '" . date_format($date_before_match, "Y-m-d") . "' ORDER BY match_idpa.match_begin";

    $match_array = $fnc->get_db_array($sql);
    ...
    // die();
    ?>
    <nav class="navbar navbar-expand-md navbar-dark fixed-top bg<?= $theme_color ?> bg-gradient">
        <div class="container-fluid">
            <a class="navbar-brand text-white" href="../shooter/index3.php">SOTH</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarCollapse" aria-controls="navbarCollapse" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarCollapse">
                <ul class="navbar-nav me-auto mb-2 mb-md-0 text-capitalize">
                    <?php if ($view_result["match_active"] == 'true' && $view_result["shooter_result"] == 'true') { ?>
                        <li class="nav-item">
                            <a class="nav-link active" href="../shooter/index3.php">My Score</a>
                        </li>
                    <?php } ?>
                    <?php if ($view_result["match_active"] == 'true' && $view_result["match_result"] == 'true') { ?>
                        <li class="nav-item">
    ...
    <main class="flex-shrink-0 mh-100" style="min-width: 100vw; width: 100vw">
        ...
        <?php
        if (isset($_GET["mid"]) && $_GET["mid"] != "") {
            if (($view_result["match_active"] == 'true' && $view_result["shooter_result"] == 'true') || $_GET["admin"] >= 5) {
        ...
        ?>
                ...
                <div class="bg-dark text-secondary px-2 px-md-4 py-5 mx-auto text-start h-100" style="max-width: 700px; min-width: 300px">
                    ...
                    ?>
                        <div class="py-3 mt-4 col-12 col-md-10 mx-auto h-100" style="max-width: 500px;">
                            ...
                            <div class="bg<?= $theme_color ?> py-3 rounded-bottom shadow text-center h5 text-white"><?= $match["match_name"] ?></div>
                            ...
                            <form action="?" method="get">
                                <div class="input-group mb-3 my-4 w-100">
                                    ...
                                    <button class="btn btn-outline<?= $theme_color ?> btn-lg px-4 fw-bold" type="submit">ดูผลการแข่งขัน</button>
                                ...
                                <div class="d-grid gap-2 d-sm-flex justify-content-sm-center" style="margin-top: 6em;">
                                    <a href="../match/shooters/?mid=<?= $mid ?>" target="_blank" class="btn btn-outline-light btn-lg px-4">ดูลำดับและรายชื่อนักกีฬา</a>
                                </div>
                            </form>
                        </div>
                    <?php } ?>
            ...
            if ($match_array) {
                echo '<div class="col-12 col-md-10 mx-auto h-100 row row-cols-2" style="margin: 5.5em 0; max-width: 800px;">';
                ...
                ?>
                    <div class="col">
                        <a href="?mid=<?= $mid ?>" target="_top"><img src="<?= $match_icon ?>" class="img-fluid box_shadow w-100"></a>
                    </div>
                <?php
                }
                echo '</div>';
        ...
        <footer class="footer mt-auto py-3 bg<?= $theme_color ?> bg-gradient text-secondary px-1 px-md-4 text-center" style="position: relative; margin-top: -56px; height: 56px; clear: both; background-color: white; min-width: 100vw; width: 100vw">
            <div class="container">
