...
function SendEmail($mail_parameter)
{
  ...
  if (!$mail->Send()) {
    ...
    // header("location:" . $mail_parameter["next_url"]);
    echo '<meta http-equiv="refresh" content="0;url=' . $mail_parameter["next_url"] . '">';
  }
}
...
// if ($_SESSION["data"]) {
  if ($_GET["type"] == "json") {
    $data = json_decode($_SESSION["data_json"], true);
