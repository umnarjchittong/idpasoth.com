...
?>
    <table class="table table-striped table-bordered table-hover table-responsive mt-2 table-light">
        ...
        <tbody>
        <?php
        $sql = "SELECT *,left(shooter_lastName,3) as shooter_num, Cast(shooter_squad As Int) as shooter_sqd FROM `score_shooter` WHERE `match_id` = " . $mid . " ORDER BY shooter_num";
        // echo $sql;
        $data_array = $fnc->get_db_array($sql);
        if (($data_array)) {
            foreach ($data_array as $row) {
                echo '<tr class="">';
                echo '<td class="text-center">' . $row["shooter_num"] . '</td>';
